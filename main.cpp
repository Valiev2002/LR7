...
void constructor(Queue&lab ) // в конструкторе мы просчто все обнуляем
{
    lab.first = nullptr;
    lab.last = nullptr;
    lab.count = 0;
...
void destructor (Queue& lab)// деструктор
{
    while (lab.first != nullptr)//пока первый элеиент не равен 0
    {
        element* temp = temp = lab.first;//присваиваем значение временной переменной
        lab.first = lab.first -> q_next;
        delete temp;//удаляем ,потому что это обязательно для динамического выделения памяти
        lab.count --;//уменьшаем на 1
    }
}
void push(Queue& myQueue, element& bmw)// функция заполнения
{
    auto* q = new element;// динамически выделяем память
    q -> value = bmw.value; //обращение к элементу значения
